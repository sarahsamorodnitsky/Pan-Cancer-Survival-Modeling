...
PosteriorLikelihood = function(betas, shape, X, Y, Last_Contact, Training_obs) {
  ...
  LogWeibullLikelihood = function(x_ij, y_ij, y_ijc, betas_it, shape_t) {
    # For testing
    ...
    if (!is.na(y_ij)) { # y_ij is not censored
      ...
      post_ij = log(pweibull(y_ijc, shape = shape_t, scale = scale_ij, lower.tail = FALSE)) 
    }
...
Model.weib = foreach(cv_iter = 1:5, .packages = c("rjags")) %dopar%{
  ...
  jags.samples(mod.full.cv_iter, c('beta', 'betatilde', 'g', 'shape'), 
...
Genes = colnames(F27.50.3_S[[1]])


cl <- makeCluster(12)
registerDoParallel(cl)
PL.weib = foreach(cv_iter = 1:5, .combine = c, .packages = c("rjags")) %dopar%{
  s.cv_iter = Model.weib[[cv_iter]]
  F5Training_obs = Generate5FoldTrainingSet(F27.50.3, S27.50.3, cancer_types_27, n_vec) # the training sets
  ...
  PosteriorLikelihood(betas = betas, shape = s.cv_iter$shape, 
                      X = F27.50.3, Y = S27.50.3, Last_Contact_27, Training_obs_cv_iter)
}
stopCluster(cl)
