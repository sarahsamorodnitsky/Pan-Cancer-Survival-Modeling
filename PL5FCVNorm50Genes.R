...
# Generates 5 folds of training values for each cancer type using a seed
...
SubsetNGenes = function(F27.50.2, Genes) {
  ...
  n = length(F27.50.2)
  F28.N.2 = F27.50.2 # note the difference here - N instead of 55
  for (i in 1:n) {
...
# Running the calculation in two steps for additional clarity 
# Also add time saving statements around both chunks to see how long entire process takes
start <- Sys.time()
cl <- makeCluster(12)
registerDoParallel(cl)
Models.N = foreach(cv_iter = 1:5, .packages = c("MASS", "truncnorm", "EnvStats")) %dopar% {
...
cl <- makeCluster(12)
registerDoParallel(cl)
PL.N = foreach(cv_iter = 1:5, .combine = c, .packages = c("MASS", "truncnorm", "EnvStats")) %dopar% {
  PosteriorLikelihood(Models.N[[cv_iter]], F27.50.3, S27.50.3)
}
stopCluster(cl)
end <- Sys.time()
end - start
