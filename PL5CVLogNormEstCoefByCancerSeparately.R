...
ModelOnTrainingData = function(Full_Input_C, Survival_Input_C, Last_Contact, sigma2_start, genes,
  ...
  names(betas) = names(X)
  beta_tilde = matrix(beta_tilde_start, ncol = p, nrow = iters + 1) # list to include posterior mean for age covariate and mutation status covariate
  lambda2 = matrix(lambda2_start, ncol = p, nrow = iters + 1)
  sigma2 = c()
...
FS28.50 = PreProcessing(F28.50.2, S28.50.2, cancer_types_28)
F28.50.3 = FS28.50$Full; S28.50.3 = FS28.50$Survival; Last_Contact = FS28.50$LastContact

F27.50.3 = F28.50.3[!(names(F28.50.3) %in% "MESO")]
S27.50.3 = S28.50.3[!(names(S28.50.3) %in% "MESO")]
Last_Contact = Last_Contact[!(names(Last_Contact) %in% "MESO")]
cancer_types_27 = cancer_types_28[cancer_types_28 != "MESO"]
...
Models.LN = foreach(cv_iter = 1:5, .packages = c("MASS", "truncnorm", "EnvStats")) %dopar% {
  ...
  # PosteriorLikelihood(Model, F27.54.3, S27.54.3)
}
stopCluster(cl)
end <- Sys.time()
end - start
...
betas = test$betas
acc.b = betas$ACC
betas.mat = matrix(unlist(acc.b), byrow = T, ncol = 52)
betas.mat.bi = betas.mat[40000:50000,]
plot(betas.mat.bi[,6], type = 'l')
